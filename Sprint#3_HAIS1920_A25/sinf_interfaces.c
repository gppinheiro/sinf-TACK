...
void create_tables (void) {
    ...
    PQexec(conn,"CREATE TABLE IF NOT EXISTS energy ( location VARCHAR(10), current DOUBLE PRECISION, voltage DOUBLE PRECISION, power DOUBLE PRECISION, time TIMESTAMP );");
...
int nmr_motes(int terminal) {
    ...
    else if (terminal==0) {fp=fopen("MsgCreatorConf.txt","r");}
...
float real_current (int C) {
    float aux, aux1;
    aux=(float)(C/4096);
    aux1=(float)(1.5/100000);
    float current=(float) 0.769 * 100000 * (aux*aux1)*1000;
    return current;
}

float current (int C) {
    float current=(float) C*0.3;
    return current;
}

//- POWER, W
float dissipated_power(float v, float i) {
    float potd=(float) v*i;
    return potd;
}
...
void* readSensors(void* arg) {
    ...
    else if (str_terminal->terminal==0) {motes_nmr=nmr_motes(0);}
    ...
    for(int i=0; i<motes_nmr; i++) {
        ...
        id_da_mote=strtol(mote_id,NULL,16);
        ...
        else if (id_da_mote==10) { id = 0; }
        else if (id_da_mote==11) { id = 1; }
        else if (id_da_mote==12) { id = 3; }
        else if (id_da_mote==13) { id = 6; }
        else if (id_da_mote==14) { id = 7; }
        else if (id_da_mote==15) { id = 5; }
        else if (id_da_mote==16) { id = 8; }
        else if (id_da_mote==17) { id = 9; }
        else if (id_da_mote==18) { id = 2; }
        else if (id_da_mote==19) { id = 4; }
        ...
        if(str_terminal->terminal==0) { sensors[id].current = strtol(sensor1,NULL,16); }
...
void* update_sensor(void *arg) {
    ...
    for(int i=0; i<NUMBER_ROOMS; i++) {
        ...
        pthread_mutex_lock(&mutex_db);
        sprintf(sql_sensor_historic,"INSERT INTO energy (location,current,voltage,power,time) VALUES ('%s','%f','%f','%f','%d-%d-%d %d:%d:%d');",what_name(id),room[id].current,room[id].voltage,room[id].power,tm.tm_year+1900,tm.tm_mon+1,tm.tm_mday,tm.tm_hour,tm.tm_min,tm.tm_sec); 
        PQexec(conn,sql_sensor_historic);
        pthread_mutex_unlock(&mutex_db);
        pthread_mutex_unlock(&mutex_room);
    }
...
void* true_sensors_values(void* arg) {
    ...
    for(int i=0; i<NUMBER_ROOMS; i++) {
        ...
        if (sensors[i].id==0) {
            ...
            room[0].voltage=220;
            room[0].current = current(sensors[0].current);
            room[0].power = dissipated_power(220,room[0].current);
            ...
            room[1].voltage=220;
            room[1].current = current(sensors[1].current);
            room[1].power = dissipated_power(220,room[1].current);
            ...
            room[3].voltage=220;
            room[3].current = current(sensors[3].current);
            room[3].power = dissipated_power(220,room[3].current);
            ...
            room[5].voltage=220;
            room[5].current = current(sensors[5].current);
            room[5].power = dissipated_power(220,room[5].current);
            ...
            room[6].voltage=220;
            room[6].current = current(sensors[6].current);
            room[6].power = dissipated_power(220,room[6].current);
            ...
            room[7].voltage=220;
            room[7].current = current(sensors[7].current);
            room[7].power = dissipated_power(220,room[7].current);
            ...
            room[8].voltage = 220;
            room[8].current = current(sensors[home[6].sensores[1][3] - '0'].current);
            ...
            room[9].voltage = 220;
            room[9].current = current(sensors[home[7].sensores[3][3] - '0'].current);
            ...
            room[2].voltage=220;
            room[2].current = current(sensors[2].current);
            room[2].power = dissipated_power(220,room[2].current);
            ...
            room[4].voltage=220;
            room[4].current = current(sensors[4].current);
            room[4].power = dissipated_power(220,room[4].current);
